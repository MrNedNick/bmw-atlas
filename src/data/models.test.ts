...
it('advertises electric facts only where engine records exist',()=>{expect(families.filter(f=>familyMatches(f,{...EMPTY_FILTERS,fuel:'Электро'})).map(f=>f.id)).toEqual(['bmw-5-series']);expect(families.filter(f=>familyMatches(f,{...EMPTY_FILTERS,fuel:'Plug-in hybrid'}))).toEqual([]);});
