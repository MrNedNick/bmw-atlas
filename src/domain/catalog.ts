...
export interface Volume {
  value: number;
  relation: "около" | "более" | "менее";
  metric: "произведено" | "продано";
  scope: string;
  asOf: string;
  source: string;
}
...
export function familyMatches(
  ...
  if (
    !matchesText(searchable, filters.query) ||
    (filters.body && !f.body.includes(filters.body)) ||
    (filters.country && !f.countries.includes(filters.country)) ||
    (filters.savedOnly && !saved.includes(f.id))
  )
    return false;
  return f.generations.some(
    (g) =>
      (!filters.year ||
        (g.start <= Number(filters.year) &&
          (g.end === null || g.end >= Number(filters.year)))) &&
...
export function parseSelection(raw: string | null, allowed: string[]) {
  return [
    ...new Set((raw ?? "").split(",").filter((id) => allowed.includes(id))),
  ].slice(0, 4);
}
export function validateCatalog(
  ...
  const source = (id: string) => {
    if (!sourceIds.has(id)) errors.push(`unknown source: ${id}`);
  };
  const volume = (v: Volume | null) => {
    if (!v) return;
    source(v.source);
    if (!Number.isFinite(v.value) || v.value < 0 || !v.scope || !v.asOf)
      errors.push("invalid volume");
  };
  for (const s of sources) {
    addId(s.id);
    if (!/^https:\/\//.test(s.url) || !s.date || !s.scope)
      errors.push(`invalid source: ${s.id}`);
  }
    ...
    for (const g of f.generations) {
      ...
      for (const p of g.powertrains) {
        ...
        if (
          ...
          !p.market ||
          !p.asOf ||
          (p.torque !== null && p.torque < 0)
        )
          errors.push(`invalid powertrain: ${p.id}`);
      }
      if (
        g.photo &&
